...
// ---------- Rest & Chat 모드 - 랜덤 별똥별 ----------
const REST_MAP_W = 1400, REST_MAP_H = 760;
const REST_STAR_MAX = 2;
const REST_STAR_SPAWN_CHANCE = 0.00008;
const REST_STAR_PICKUP_RADIUS = 40;
const REST_FIRE_LIGHT = { x: REST_MAP_W/2 - 20, y: REST_MAP_H/2 - 40 + 85 - 30, radius: 175 };
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'restMove') {
      ...
      for (const [starId, star] of room.restStars) {
        if (Math.hypot(msg.x - star.x, msg.y - star.y) < REST_STAR_PICKUP_RADIUS) {
          room.restStars.delete(starId);
          broadcastAll(room, { type: 'restStarCollected', id: starId, collectorId: ws.playerId, collectorName: me.name });
          break;
        }
      }
      return;
